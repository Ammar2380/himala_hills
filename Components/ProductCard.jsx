...
const ProductCard = ({ product, onSelect }) => {
  // Use .originalPrice to match your PRODUCTS data file
  const mainVariant = product.variants?.[0] || { price: 0, originalPrice: 0, name: 'N/A' };
  ...
  const oldPrice = mainVariant.originalPrice || 0;
  ...
  return (
    <>
      ...
      >
        ...
        <h3 className="font-semibold text-[17px] mb-2 leading-snug px-1 text-gray-900">{product.name}</h3>
        ...
        <div className="flex items-center gap-3 mb-3 px-1">
          <span className="bg-yellow-400 px-3 py-1 rounded-lg font-black text-lg text-gray-900">
      ...
      >
        ...
        {/* Product Image Stage */}
        <div className="relative bg-[#F8F8F8] rounded-[2rem] overflow-hidden flex items-center justify-center">
          <motion.img
            src={getProductImage(product.id)}
            alt={product.name}
        ...
        <div className="px-2 pt-4 pb-2">
          ...
          >
            <span className="relative z-10 uppercase">Add to Cart — {mainVariant.name}</span>
            <motion.div 
